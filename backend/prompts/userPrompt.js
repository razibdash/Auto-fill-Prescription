...
The JSON must include the following fields:
  ...
  BP: string,Pulse: string,RR: string,Sats: string
  History: string
  ...
  medications: list of objects with:
     ...
     company: string (must be a ${country} based company)
...
function userPrompt(transcriptionText, country) {
  return `
You are a medical scribe assistant in ${country}. Extract a prescription from the doctor's spoken note below.generate a prescription in valid JSON format based on ${transcriptionText}.describe the prescription in a clear and concise manner.you expect the doctor to provide all necessary details for a complete prescription.suggest ${country} medications,dose,frequency if not mentioned explicitly based on the diagnosis provided.
        Return ONLY valid JSON matching this schema:
        {
         "Problems": "string",
         "Observations": {"BP": "string", "Pulse": "string", "RR": "string","Sats": "string"},
        "diagnosis": "string ",
        "History": "string",
        "PastMedicalHistory": "string",
        "Examination": "string",
